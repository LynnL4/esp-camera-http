...
void wifi_init(void)
{
    ...
    int bits = xEventGroupWaitBits(wifi_event_group, STA_STARTED_BIT, pdFALSE, pdTRUE, JOIN_TIMEOUT_MS / portTICK_PERIOD_MS);
    ...
    {
        {
            ...
            ESP_LOGI(TAG, "bssid: %02x:%02x:%02x:%02x:%02x:%02x", wifi_config.sta.bssid[0], wifi_config.sta.bssid[1], wifi_config.sta.bssid[2], wifi_config.sta.bssid[3], wifi_config.sta.bssid[4],
...
static esp_err_t wifi_sta_join_op(wifi_op_t *self, int argc, char *argv[])
{
    ...
    int bits = xEventGroupWaitBits(wifi_event_group, CONNECTED_BIT, pdFALSE, pdTRUE, JOIN_TIMEOUT_MS / portTICK_PERIOD_MS);
...
{
    wifi_config_t wifi_config = { 0 };
    strlcpy((char *)wifi_config.sta.ssid, "ASUS", sizeof(wifi_config.sta.ssid));
    strlcpy((char *)wifi_config.sta.password, "admin856", sizeof(wifi_config.sta.password));
    wifi_init();
    ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config));
    esp_wifi_connect();

    esp_err_t ret;
    esp_console_cmd_t command = { .command = "wifi", .help = "Command for wifi configuration and monitoring\n For more info run 'wifi help'", .func = &do_cmd_wifi };
